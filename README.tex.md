...
* The change $\Delta z$ during a small period of time $\Delta t$ is: $\Delta z = \epsilon\sqrt{\Delta t}$ where $\epsilon$ has a normal distribution of $\phi(0,1)$. So $\Delta z$ will have a mean of $0$ and a variance of $\Delta t$. It can be seen [here](https://github.com/joseprupi/randomwalk) that for a variale $X$ that can take values $1$ and $-1$ witha a probability of $\frac{1}{2}$ each (random walk) $\frac{1}{\sqrt{n}}\left( X_{1}+\cdots +X_{n} \right)\ {\xrightarrow {}}\ N\left(0,1\right) $ 
